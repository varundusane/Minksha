...
theme.Header = (function() {
  ...
  function submenuParentClickHandler(event) {
    ...
    if(element.classList.contains('menu-item')) return; //CD
...
theme.MobileNav = (function() {
  ...
  function openMobileNav() {
    ...
    //CD
    var site_header = document.querySelector('.header-wrap .site-header');
    if(site_header != null) {
      site_header.classList.add('nav-open');
    }
  ...
  function closeMobileNav() {
    ...
    var site_header = document.querySelector('.header-wrap .site-header');
    if(site_header != null) {
      site_header.classList.remove('nav-open');
    }
...
theme.SearchDrawer = (function() {
  ...
  function setHeight() {
    ...
    //CD
    if(headerHeight == 0) {
      var header = document.querySelector('.header-wrap .site-header');
...
window.onbeforeunload = function() {
  $('.pre-loader').removeClass('disappear');
  $('body').removeClass('loaded');
}
$(window).on('load',function(){
  $('.pre-loader').addClass('disappear');
  $('body').addClass('loaded');
  
  if($('.home-main').length) {
    setTimeout(function(){
      $('.home-main').addClass('loaded');
    },1000);
  }
});
...
$(function(){
  $('.faq-que').on('click',function(){
    ...
    $parent.siblings('.faq').find('.faq-ans').slideUp();
  ...
  if($(window).width() > 749) {
    $('.img-slider').each(function(){
      var $this = $(this);
      var sec_id = $this.attr('data');
      $this.slick({
        slidesToShow: 4,
        prevArrow: $this.parents('.slider-wrap').find('.btn-prev'),
        nextArrow: $this.parents('.slider-wrap').find('.btn-next'),
        responsive: [
          {
            breakpoint: 1200,
            settings: {
...
$(document).ready(function(){
  if($('.small-header-fixed').length && $(window).width() < 750) {
    if(!$('.small-header-fixed').hasClass('header-transparent') && !$('.small-header-fixed').hasClass('index-header')) $('.header-back').css('height',$('.small-header-fixed').outerHeight());
    if($('.small-header-fixed').hasClass('index-header')) {
      if($('.slide-content').length) $('.slide-content').css('padding-top',$('.small-header-fixed').outerHeight());
    }
  }
...
$('.lnk-to-slide').on('click',function(e){
  e.preventDefault();
  $($(this).attr('href')).addClass('active').siblings('.slide-content').removeClass('active');
  $('.main-slides').attr('index',1);
  setTimeout(function(){
    $('html,body').animate({scrollTop: 0});
  },510);
});

$('.back-lnk').on('click',function(e){
  e.preventDefault();
  $('.main-slides').attr('index',0);
  setTimeout(function(){
    $('html,body').animate({scrollTop: 0});
...
/*$('.jsNav[href*="#"]').on('click',function(e){
  e.preventDefault();
...
function checkPosition() {
  for (var i = 0; i < elements.length; i++) {
    ...
    if (positionFromTop - parseInt(windowHeight/1.5) <= 0) {
...
function checkCookie() {
  var user=getCookie("formcookie");
  if (user == "") {
    setTimeout(function(){$('.popup_wrapper').addClass('show');}, 3000);
  }
}